...
Stateful components holds state while a functional component receives input via props.

2. When does a componentWillMount function be called? What about a componentWillUpdate?

componentWillMount function will run on mount of the component or when the component first renders. componentWillUpdate runs when the state is changed.

3. Define stateful logic.

Stateful logic is any code that manipulates or changes state.

4. What are the three step of creating a successful test? What is done in each phase?

Arrange - rendering the element in the DOM
Act - Select and manipulate the nodes from the dom through methods such as screen.getByLabelText or using userEvent
Assert - Confirm the nodes have rendered. Using expect calling on the selected nodes in Act, and then .toBeInTheDocument.
